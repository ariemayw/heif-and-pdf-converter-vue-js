<template>
  <div class="converter-card">
    <h2>Word to PDF</h2>
    <p class="description">Convert your Word documents (.doc, .docx) to PDF format</p>
    ...
    <div class="upload-area" :class="{ 'drag-over': isDragging }">
      <input
        ...
        id="word-file"
      />
      <label for="word-file" class="upload-label">
        <div class="upload-icon">📄</div>
        <p v-if="!file">Click to upload or drag and drop</p>
        <p v-else class="file-name">{{ file.name }}</p>
        <span class="file-hint">Supported: .doc, .docx (Max 10MB)</span>
      </label>
    </div>
